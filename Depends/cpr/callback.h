...
namespace cpr {
	...
	public:
		...
		// NOLINTNEXTLINE(google-explicit-constructor, hicpp-explicit-conversions)
		DebugCallback(std::function<void(InfoType T, std::string data, intptr_t userdata)> p_callback, intptr_t p_userdata = 0) : userdata(p_userdata), callback(std::move(p_callback)) {}
		void operator()(InfoType T, std::string data) const {
			callback(T, std::move(data), userdata);
		}

		intptr_t userdata{};
		std::function<void(InfoType T, std::string data, intptr_t userdata)> callback;
	};
} // namespace cpr
