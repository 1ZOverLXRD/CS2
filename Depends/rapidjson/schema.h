...
namespace internal {
	...
	private:
		...
		bool WriteType(Type T) { return WriteBuffer(T, 0, 0); }

		bool WriteNumber(const Number& n) { return WriteBuffer(kNumberType, &n, sizeof(n)); }

		bool WriteBuffer(Type T, const void* data, size_t len) {
			// FNV-1a from http://isthe.com/chongo/tech/comp/fnv/
			uint64_t h = Hash(RAPIDJSON_UINT64_C2(0xcbf29ce4, 0x84222325), T);
...
#undef RAPIDJSON_STRING_
	...
	private:
		...
		void AddType(const ValueType& T) {
			if (T == GetNullString()) type_ |= 1 << kNullSchemaType;
			else if (T == GetBooleanString()) type_ |= 1 << kBooleanSchemaType;
			else if (T == GetObjectString()) type_ |= 1 << kObjectSchemaType;
			else if (T == GetArrayString()) type_ |= 1 << kArraySchemaType;
			else if (T == GetStringString()) type_ |= 1 << kStringSchemaType;
			else if (T == GetIntegerString()) type_ |= 1 << kIntegerSchemaType;
			else if (T == GetNumberString()) type_ |= (1 << kNumberSchemaType) | (1 << kIntegerSchemaType);
		}
