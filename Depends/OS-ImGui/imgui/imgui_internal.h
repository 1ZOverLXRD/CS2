...
namespace ImGui
{
	...
	IMGUI_API void          CallContextHooks(ImGuiContext* context, ImGuiContextHookType T);
	...
	// Logging/Capture
	IMGUI_API void          LogBegin(ImGuiLogType T, int auto_open_depth);           // -> BeginCapture() when we design v2 api, for now stay under the radar by using the old name.
