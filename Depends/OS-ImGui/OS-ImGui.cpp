...
namespace OSImGui
{
	ID3D11ShaderResourceView* OSImGui::LoadTextureFromFile(const char* filename, int& out_w, int& out_h)
	{
		return LoadTextureFromFile_external(filename, out_w, out_h);
	}
	void OSImGui::DrawImage(ID3D11ShaderResourceView* texture, const ImVec2& pos, const ImVec2& size) {
		ImTextureID texID = (ImTextureID)texture;

		// 定义图像的 UV 坐标（通常为 (0,0) 到 (1,1)）
		ImVec2 uv0(0.0f, 0.0f); // 左上角
		ImVec2 uv1(1.0f, 1.0f); // 右下角

		// 定义图像的颜色（通常为白色，表示不修改图像颜色）
		ImVec4 tintColor(1.0f, 1.0f, 1.0f, 1.0f); // RGBA

		// 使用 ImDrawList 绘制图像
		ImGui::GetBackgroundDrawList()->AddImage(texID, pos, ImVec2(pos.x + size.x, pos.y + size.y), uv0, uv1, ImColor(tintColor));
	}
	void OSImGui::Text(std::string Text, Vec2 Pos, ImColor Color, float FontSize, bool KeepCenter)
	{
		...
		// 逐行绘制文本
		float textHeight = ImGui::GetFont()->FontSize;  // 文字高度，取决于字体大小
		float totalHeight = textHeight * lines.size();  // 总高度，所有行的总和

		// 如果需要居中对齐
		if (KeepCenter)
		{
			float textWidth = ImGui::GetFont()->CalcTextSizeA(FontSize, FLT_MAX, 0.f, Text.c_str()).x;
			Pos.x -= textWidth / 2;  // 水平居中
			Pos.y -= totalHeight / 2;  // 垂直居中
		}

		// 绘制每一行
		for (size_t i = 0; i < lines.size(); ++i)
		{
			ImGui::GetBackgroundDrawList()->AddText(ImGui::GetFont(), FontSize, ImVec2(Pos.x, Pos.y + i * textHeight), Color, lines[i].c_str());
		}
	}
